...
all_interest_regions <- cov_deseq %>% select(chro, start, end)
...
sig_interest_regions <- cov_deseq_sig %>% select(chro, start, end)

# save the the tables that contain all and significant regions of interest in a text format
write.table(all_interest_regions, file="output/output3_extract_interest_regions/all_interest_regions.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
write.table(sig_interest_regions, file="output/output3_extract_interest_regions/sig_interest_regions.txt", row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
